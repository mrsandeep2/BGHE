    ...
    <div className="min-h-screen">
      ...
      <section className="py-16 md:py-20 bg-background border-y border-border/60">
        <div className="container mx-auto px-4">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            <SectionHeading
              title="Bihar Admission Guides"
              subtitle="Explore the core BGHE search pages built for Bihar students looking for admission help, DRCC guidance, and higher education support."
            />
          </motion.div>
          <div className="grid md:grid-cols-2 xl:grid-cols-5 gap-4 mt-10">
            {seoHubLinks.map((link, i) => (
              <motion.div key={link.path} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08 }}>
                <Link to={link.path}>
                  <Card className="h-full border-0 shadow-md hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
                    <CardContent className="p-5">
                      ...
                      <h2 className="font-heading font-bold text-lg text-foreground mb-2">{link.label}</h2>
                      <p className="text-sm text-muted-foreground leading-6 mb-4">{link.description}</p>
                      <span className="text-sm font-semibold text-accent inline-flex items-center gap-1">Open Page <ArrowRight className="w-4 h-4" /></span>
                    </CardContent>
                  </Card>
                </Link>
              </motion.div>
